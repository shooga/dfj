...
function ourplan(X,Y){
    var imagesrc="image/my.png";
    plan.call(this,1,X,Y,66,80,0,660,0,"image/boom.png",imagesrc);
}
...
var onMouseMove=function(){
    ...
    selfplan.imagenode.style.left=selfplanX-selfplan.plansizeX/2+"px";
    selfplan.imagenode.style.top=selfplanY-selfplan.plansizeY/2+"px";
}
/*
暂停事件
 */
...
var onOutSide=function(){
    ...
    if(bodyobjX<505||bodyobjX>815||bodyobjY<0||bodyobjY>568){
        ...
        if(document.addEventListener){
            ...
            mainDiv.attachEvent("nomousemove",onMouseMove);
        }
    }
}
...
function initListeners()
{
	if(document.addEventListener){
		...
		//为暂停界面的返回主页按钮添加事件
		suspenddiv.getElementsByTagName("button")[1].attachEvent("click",restartGame,true);
	}
}
...
function start(){
    ...
    if(mark==20){
        ...
        //中飞机
        if(mark1%5==0){
            enemys.push(new enemy(6,25,274,46,60,5000,360,random(1,3),"image/boom.png","image/small2.png"));
        }
        //大飞机
        if(mark1==20){
            enemys.push(new enemy(12,57,210,110,107,30000,540,1,"image/boom.png","image/boss.png"));
            mark1=0;
        }
        //小飞机
        else{
            enemys.push(new enemy(1,19,286,34,24,1000,360,random(1,4),"image/boom.png","image/small.png"));
        }
        mark=0;
    }
...
function continueGame(){
	...
	mainDiv.removeChild(selfplan.imagenode);
	
	// 重新初始化
	selfplan.init();
	selfplan.imagenode.src = "image/my.png";
